...
public class LoginProcessController {
	...
	public String loginProcess(HttpServletRequest request
		...
		try{
			...
			Log.m_PathName = SdiagProperties.getProperty("log_path");;
			Log.m_FileName = "Login";
			
			
			Log.TraceLog("[LoginRequset : "+loginID +"] [IP : "+ CommonUtil.getLocalIPAddress(request)+"]");
			...
			if(request.getParameter("u1").equals("admin")){
				...
				if(isLdap){
					...
					if(result==1){
						Log.TraceLog("[LDAP_before : "+request.getParameter("u1").toString()+"] [IP : "+ CommonUtil.getLocalIPAddress(request)+"]");
						...
						if(isLogin){
							 ...
							 * 로그인성공이면 로그인실패회수 초기화
							 */
							Log.TraceLog("[LDAP_after : "+request.getParameter("u1").toString()+"] [IP : "+ CommonUtil.getLocalIPAddress(request)+"]");
							comService.LoginFailCountDelete(loginID);
						}else{
							map.put("result", "102");
			...
			case 5:
				...
				String uid = (String)request.getParameter("u1");
				Log.TraceLog("[Security_before : "+request.getParameter("u1").toString()+"] [IP : "+ CommonUtil.getLocalIPAddress(request)+"]");
				//인증암호 - Ldap통한 암호 확인 불가로 기본값 입력함 
				...
				Log.TraceLog("[Security_after : "+request.getParameter("u1").toString()+"] [IP : "+ CommonUtil.getLocalIPAddress(request)+"]");
				
				/*****************************************************************
				 * 접속 로그  필요시 적용
				 *****************************************************************/
				MemberVO loginInfo = CommonUtil.getMemberInfo();
				Log.TraceLog("[Session_userId : "+loginInfo.getUserid()+"] [IP : "+ CommonUtil.getLocalIPAddress(request)+"]");
				
				if(!loginID.equals(loginInfo.getUserid())){
					request.getSession().invalidate();
					rUrl = "forward:/loginPage.do";
					Log.TraceLog("로그인 불가 -[Request_ID : "+request.getParameter("u1").toString()+"] [Session_userId : "+loginInfo.getUserid()+"] [IP : "+ CommonUtil.getLocalIPAddress(request)+"]");
					break;
				}
